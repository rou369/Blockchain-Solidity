...
contract('Dex', () => {
    ...
    [DAI, BAT, REP, ZRX] = ['DAI', 'BAT', 'REP', 'ZRX']
        .map(ticker => web.utils.fromAscii(ticker));
    beforeEach(async () => {
        ...
        dex = await Dex.new()
        await Promise.all([
            dex.addToken(DAI, dai.addToken),
            dex.addToken(BAT, bat.addToken),
            dex.addToken(REP, rep.addToken),
            dex.addToken(ZRX, zrx.addToken)
        ]);
